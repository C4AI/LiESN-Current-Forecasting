...
def train_deepesn(steps):
    ...
    sweep_config = {
        ...
        'parameters': {
            #ADCP
            'spectral_radius_1': {'max': 1.0, 'min': 0.0001},
            'leak_rate_1': {'max': 1.0, 'min': 0.0000},
            'reservoir_size_1': {'values': [x for x in range(300, 7100, 100)]},
            'connectivity_1': {'max': 0.8, 'min': 0.0001},
            'input_scaling_1': {'max': 5.0, 'min': 0.0001},
            'bias_scaling_1': {'max': 5.0, 'min': 0.0000},
            #SOFS
            'spectral_radius_2': {'max': 1.0, 'min': 0.0001},
            'leak_rate_2': {'max': 1.0, 'min': 0.0000},
            'reservoir_size_2': {'values': [x for x in range(300, 7100, 100)]},
            'connectivity_2': {'max': 0.8, 'min': 0.0001},
            'input_scaling_2': {'max': 5.0, 'min': 0.0001},
            'bias_scaling_2': {'max': 5.0, 'min': 0.0000},
            #MERGER
            'spectral_radius_3': {'max': 1.0, 'min': 0.0001},
            'leak_rate_3': {'max': 1.0, 'min': 0.0000},
            'reservoir_size_3': {'values': [x for x in range(300, 7100, 100)]},
            'connectivity_3': {'max': 0.8, 'min': 0.0001},
            'input_scaling_3': {'max': 5.0, 'min': 0.0001},
            'bias_scaling_3': {'max': 5.0, 'min': 0.0000},
    ...
    try:
        # Create new Sweep if  no sweep was defined
...
def config_2_dict(**config):
    ...
    dict_train = {"layers": {
        "Layer1": {"ADCP": {
            'spectral_radius': config['spectral_radius_1'],
            'leak_rate': config['leak_rate_1'],
            'reservoir_size': config['reservoir_size_1'],
            'connectivity': config['connectivity_1'],
            'input_scaling': config['input_scaling_1'],
            'bias_scaling': config['bias_scaling_1'],
            ...
            "SOFS": {
                'spectral_radius': config['spectral_radius_2'],
                'leak_rate': config['leak_rate_2'],
                'reservoir_size': config['reservoir_size_2'],
                'connectivity': config['connectivity_2'],
                'input_scaling': config['input_scaling_2'],
                'bias_scaling': config['bias_scaling_2'],
                'time_constant': config['time_constant'],
                'ridge_parameter': config['ridge_parameter'],
                'input_dim': 1,
                'output_dim': 1
        ...
        "Layer2": {"Joiner": {
            'spectral_radius': config['spectral_radius_3'],
            'leak_rate': config['leak_rate_3'],
            'reservoir_size': config['reservoir_size_3'],
            'connectivity': config['connectivity_3'],
            'input_scaling': config['input_scaling_3'],
            'bias_scaling': config['bias_scaling_3'],
            'time_constant': config['time_constant'],
            'ridge_parameter': config['ridge_parameter'],
            'input_dim': config['reservoir_size_1'] + config['reservoir_size_2'],
            'output_dim': 1
        },
        },
    },
    ...
    try:
        ...
        with wandb.init(config=config, project=train_parameters['Project_name'], entity=train_parameters['username']):
            ...
            train_ds, validate_ds, dict_train['input_dim'], dict_train['output_dim'], dict_train[
                'out_maps'], dict_train['input_map'] = train_dl.split_train_val(val_per=0.2,        # 20% data for validation
            ...
            # Only consider losses of the ADCP predictions (dim:0)
            fold_losses = np.nanmean(losses[:,0])
            fold_std = np.nanstd(losses[:,0])
...
def evaluate():
    ...
    # Recover trained parameters from wandb
    try:
    ...
    # Training dataset with 100% of the data
    train_dl = DataSplitter(**d)
    train_ds, validate_ds, dict_train['input_dim'], dict_train['output_dim'], dict_train[
        'out_maps'], dict_train['input_map'] = train_dl.split_train_val(val_per=0.0,
    ...
    if 'missing_hours' in train_parameters.keys():
        test = test_dl.sequential_batches_missing_before_forecast(batch_duration=train_parameters["forecast_horizon"] + train_parameters["warmup"],
                                                                  stride=train_parameters["forecast_horizon"],
                                                                  forecast_window=train_parameters['forecast_horizon'],
                                                                  missing_hours=train_parameters['missing_hours'])
    else:
    ...
    if 'missing_hours' in train_parameters.keys():
        print('Missing data for gap' + str(int(train_parameters["missing_hours"])) + ' hours')
    ...
    # Dump predictions results
    if 'missing_hours' in train_parameters.keys():
        with open(path + str(int(train_parameters["missing_hours"])) + '_gap_missing.pickle', 'wb') as handle:
            dta = inputs, predictions, losses
            pickle.dump(dta, handle, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open(path + 'no_missing.pickle', 'wb') as handle:
            dta = inputs, predictions, losses
            pickle.dump(dta, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == '__main__':
    ...
    # train/validate dict
    d = {'timeseries': ({'path': 'data/train/current_projection_praticagem.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [['low_pass_filter', 2]], 'description': 'ADCP', 'allow_missing':True, 'missing_threshold': 60, 'blackout': True},
                        {'path': 'data/train/current_sofs_praticagem_15min.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [], 'description': 'SOFS', 'allow_missing':True, 'missing_threshold': 180, 'blackout': True}),
         'device': torch.device('cuda'), 'dtype': torch.float32}


    # test dict
    d_test = {'timeseries': ({'path': 'data/test/current_projection_praticagem.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [['low_pass_filter', 2]], 'description': 'ADCP', 'allow_missing':True, 'missing_threshold': 60, 'blackout': True},
                        {'path': 'data/test/current_sofs_praticagem_15min.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [], 'description': 'SOFS', 'allow_missing':False, 'missing_threshold': 180, 'blackout': True}),
         'device': torch.device('cuda'), 'dtype': torch.float32}

    # path where the weights will be saved locally
    train_parameters = json.load(open("esn_config_architecture_C.json", "r"))
