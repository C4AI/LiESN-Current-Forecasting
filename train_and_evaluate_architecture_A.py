...
def train_esn(steps):
    ...
    try:
        ...
        wandb.agent(sweep_id=train_parameters['Sweep_id'], project=train_parameters['Project_name'], function=train, count=steps)
...
def evaluate():
    ...
    # Retrain the ESN with all the train/validate dataset
    try :
    ...
    print('mean losses test:' + str(np.mean(losses_test)) + '+-' + str(np.std(losses_test)))
...
if __name__ == '__main__':
    ...
    d = {'timeseries': ({'path': 'data/train/current_projection_praticagem.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [['low_pass_filter', 2]], 'description': 'ADCP', 'allow_missing':True, 'missing_threshold': 60, 'blackout': True},),
         'n_folds': 5, 'device': torch.device('cuda'), 'dtype': torch.float32, 'train_batches': 100, 'val_batches': 100}
    ...
    d_test = {'timeseries': ({'path': 'data/test/current_projection_praticagem.csv', 'type': 'velocity_projection', 'is_predicted':True, 'transformations': [['low_pass_filter', 2]], 'description': 'ADCP', 'allow_missing':True, 'missing_threshold': 60,  'blackout': True},),
         'n_folds': 5, 'device': torch.device('cuda'), 'dtype': torch.float32, 'train_batches': 50, 'val_batches': 50}

    train_parameters = json.load(open("esn_config_architecture_A.json", "r"))
    ...
    if all([train_parameters['wandb_api_key'], train_parameters['username'], train_parameters['Project_name']]):
        train_esn(train_parameters['steps'])
    ...
    for m in blackout:
        train_parameters['save_path_output'] = str(int(m*100)) + '_both_missing'
